(() => {
  ...
  function drawRouteMap(id, points, color) {
    const el = $(`#${id}`);
    if (!el) return;
    const navigationMode = id === "mobile-map";
    if (!points.length) {
      el.innerHTML = `
    ...
    if (window.google?.maps) {
      ...
      if (navigationMode) {
        const heading = routeHeading(points);
      ...
      points.forEach((point, index) => {
        if (navigationMode && index === 0) return;
        new google.maps.Marker({
          map,
          position: point,
          title: point.label,
    ...
    el.innerHTML = `
      <div class="map-fallback">
        ...
        <div class="fake-car"><span class="fake-nav-arrow"></span></div>
  ...
  function routeHeading(points) {
    if (!points[0] || !points[1]) return 0;
    if (window.google?.maps?.geometry?.spherical) {
      return google.maps.geometry.spherical.computeHeading(points[0], points[1]);
    }
  ...
  function startLiveNavigation() {
    if (liveWatchId !== null || state.mobileTrip.status === "idle" || !navigator.geolocation) return;
    liveWatchId = navigator.geolocation.watchPosition(position => {
      const current = usablePoint(position.coords.latitude, position.coords.longitude);
      if (!current) return;
      state.mobileTrip.startPoint = current;
      const now = Date.now();
      if (now - lastGpsRenderAt > 3000 && state.mobilePanel === "meetings") {
        lastGpsRenderAt = now;
        initMobileMap();
      }
      saveState();
