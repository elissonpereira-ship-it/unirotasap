(() => {
  ...
  function normalizeSeller(row) {
    ...
    return {
      ...
      home: point(row.home_lat, row.home_lng) || { lat: -23.5505, lng: -46.6333 }
    };
  }
  ...
  function normalizeMeeting(row) {
    return {
      ...
      point: point(row.lat, row.lng) || { lat: -23.5505, lng: -46.6333 },
  ...
  function normalizeMessages(rows, sellers) {
    ...
    return rows.map(row => {
      const sellerId = row.sender_uid === managerUid ? row.receiver_uid : row.sender_uid;
      const from = row.sender_uid === managerUid ? "manager" : "seller";
      return {
        id: row.id,
        sellerId,
        from,
  ...
  function renderMobileHome(nextMeeting) {
    ...
    const approvalCopy = state.mobileApproved ? "Aprovado" : "Aguardando aprovacao";
  ...
  function mobileTripPoints() {
    ...
    const paxes = state.mobileTrip.passengers.map(id => seller(id)?.home).filter(Boolean);
    if (!place) return [{ kind: "real-start", label: "Voce", ...driver.home }];
    return [
      { kind: "real-start", label: "Voce", ...driver.home },
      ...paxes.map((point, index) => ({ kind: "ride", label: `Carona ${index + 1}`, ...point })),
  ...
  function mobileChatPanel() {
    const messages = state.mobileTrip.chat || [];
    return `
      <div class="mobile-card-list">
        <div class="mobile-card">
          <h4>Mensagens da viagem</h4>
          <p>Este chat desaparece quando a rota ou o embarque do carona termina.</p>
        </div>
        ${messages.map(msg => `<div class="mobile-card"><h4>${msg.from}</h4><p>${msg.text}</p></div>`).join("") || `<div class="mobile-card"><p>Sem mensagens temporarias.</p></div>`}
        <div class="mobile-card-actions">
  ...
  function mobileSafetyPanel() {
    return `
      <div class="mobile-card-list">
        <div class="mobile-card">
          <h4>Seguranca</h4>
          <p>Use emergencia para registrar pane, pneu furado, acidente, desvio ou parada nao planejada. O gestor recebe a notificacao.</p>
          <div class="mobile-card-actions">
            <button class="tiny-button danger" data-mobile-emergency="Pneu furado">Pneu furado</button>
            <button class="tiny-button danger" data-mobile-emergency="Pane mecanica">Pane mecanica</button>
          </div>
        </div>
      </div>
    `;
  ...
  function mobileHistoryPanel() {
    const me = currentSeller();
    const rows = state.data.routes.filter(item => item.driverId === me.id);
    return `
      <div class="mobile-card-list">
  ...
  async function beginTrip(role, meetingId, vehicle = "other") {
    ...
    if (role === "driver") {
      const startPoint = await getCurrentPositionPoint(me.home);
      const route = buildRouteFromMobileTrip(startPoint);
  ...
  function buildRouteFromMobileTrip(startPoint = null) {
    ...
    const passengerPoints = state.mobileTrip.passengers
      .map(sellerId => {
        ...
        return item ? { sellerId, name: item.name, ...item.home } : null;
      })
      .filter(Boolean);
    const plannedOutbound = [
      { kind: "home", label: "Casa motorista", ...driver.home },
      ...
      real: [
        {
          leg: "outbound",
          points: [
            { kind: "real-start", label: "Inicio real", ...(startPoint || driver.home) },
  ...
  function bindGlobalEvents() {
    document.addEventListener("click", async event => {
      const target = event.target.closest("button, [data-panel-jump], [data-route-detail], [data-delete-route], [data-edit-seller], [data-delete-seller], [data-approve], [data-refuse], [data-edit-meeting], [data-delete-meeting], [data-contact], [data-message-seller], [data-route-leg], [data-export-one], [data-sync-route], [data-sync-mobile], [data-open-sidebar], [data-add-passenger], [data-start-role], [data-mobile-panel], [data-quick-chat], [data-mobile-emergency]");
      if (!target) return;
    ...
    $("#mobile-locate").addEventListener("click", async () => {
      await getCurrentPositionPoint(currentSeller().home);
      showToast("GPS ativo. Ponto real do motorista atualizado.", "success");
    });
