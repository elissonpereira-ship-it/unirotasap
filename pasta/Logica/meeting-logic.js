...
async function m_startOutbound() {
    ...
    // CRIA SESSÃO NO SUPABASE PARA SINCRONIA
    ...
    const sessionData = {
        ...
        vehicle: m_state.vehicle || 'carro',
        pax_list: m_state.paxSelected.map(p => ({
            uid: p.uid,
            name: p.name,
            boarded: false,
            signal_embark: false
...
function m_resumeOutbound() {
    ...
    m_startGpsPolling();
    m_syncMeetingState(); // Inicia escuta da sessão
}

// SINCRONIZADOR DE ESTADO DA SESSÃO
function m_syncMeetingState() {
    if (!m_state.sessionId) return;

    window.supabase.database().ref(`meeting_sessions/${m_state.sessionId}`)
        .on('value', (snapshot) => {
            ...
            // LOGICA DO PASSAGEIRO: Receber sinal de embarque
            const myUid = window.currentVendorUid;
            const myInfo = data.pax_list ? data.pax_list.find(p => p.uid === myUid) : null;
            
            if (myInfo && myInfo.signal_embark && !myInfo.boarded) {
                openModal('m-modal-embark');
            }

            m_renderOutboundList(); // Atualiza a lista visual (motorista vê quem embarcou)
        });
}

// RENDERIZA LISTA DE EMBARQUE (MOTORISTA)
function m_renderOutboundList() {
    const container = document.getElementById('m-outbound-pax-list');
    if (!container) return;

    const pList = m_state.currentSession?.pax_list || m_state.paxSelected;

    container.innerHTML = pList.map(p => {
        const isOnline = p.isOnline; // Vem do polling de GPS que corre em paralelo
                ...
                <div style="display:flex; justify-content:space-between; align-items:center;">
                    <div>
                        ...
                        <div style="font-size:0.7rem; color:${isOnline ? 'var(--success)' : 'var(--danger)'};">
                            <i data-lucide="${isOnline ? 'gps' : 'gps-off'}" style="width:10px;"></i>
