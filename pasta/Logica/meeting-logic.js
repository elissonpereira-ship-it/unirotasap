...
async function m_startOutbound() {
    ...
    const sessionData = {
        ...
        vehicle_type: m_state.vehicle || 'carro',
        passengers: m_state.paxSelected.map(p => ({
            uid: p.uid,
            name: p.name,
            boarded: false,
            signal_embark: false
        })),
        date: new Date().toISOString().split('T')[0], // Coluna 'date' que vimos no log
    ...
    try {
        ...
        console.error("🚨 Erro Final ao criar sessão:", e.message);
...
// REALTIME NATIVO (AJUSTADO PARA A COLUNA 'id')
...
function m_subscribeToSession() {
    ...
    m_sessionChannel = window.supabase
        ...
        .on('postgres_changes', { 
            ...
            filter: `id=eq.${m_state.sessionId}`
        }, payload => {
            const data = payload.new;
            if (!data) return;

            m_state.currentSession = data;
            console.log("⚡ [Realtime] Sessão Atualizada:", data);
...
// MOTORISTA ENVIA SINAL (COLUNA 'passengers')
async function m_sendEmbarkSignal(paxUid) {
    ...
    try {
        ...
        console.error("Erro ao enviar sinal:", e.message);
    }
}

// PASSAGEIRO CONFIRMA (COLUNA 'passengers')
async function m_paxConfirmEmbark() {
    ...
    try {
        ...
        console.error("Erro ao embarcar:", e.message);
    }
}

// RENDERIZA LISTA DE EMBARQUE (MOTORISTA) - DADOS NATIVOS
function m_renderOutboundList() {
    const container = document.getElementById('m-outbound-pax-list');
    if (!container) return;

    // Prioriza os dados que vêm do Realtime (currentSession.passengers)
            ...
            <div class="meeting-card" style="margin-bottom:8px; opacity: ${boarded ? '0.6' : '1'}">
                <div style="display:flex; justify-content:space-between; align-items:center;">
                    ...
                    <div style="display:flex; gap:8px;">
                        ${boarded ? 
                            '<span style="color:var(--success); font-size:0.75rem; font-weight:800;">EMBARCADO</span>' :
                            `<button class="pax-action-btn" onclick="m_sendEmbarkSignal('${p.uid}')" style="background:var(--gold-bg); border:1px solid var(--gold); padding:8px; border-radius:10px; color:var(--gold);">
...
// LOOP DE GPS (INDISPENSÁVEL PARA O RADAR)
...
async function m_syncGpsLoop() {
    ...
    try {
        ...
        console.log("📡 [V1.2] GPS RADAR - Dados do Banco:", data);
