// UniRotas - Vendedor V2.6 (Totalmente Supabase - Firebase Exterminado)
...
let chatSub = null; // Canal de Realtime do Supabase
...
function updateNextMeetingWidget() {
    ...
    if (now.getDate() > nextMeeting.getDate()) nextMeeting = getFirstBizDay(now.getFullYear(), now.getMonth() + 1);
    ...
    const dateEl = document.getElementById('next-meeting-date-text'), daysEl = document.getElementById('next-meeting-days-count');
...
function showScreen(id) {
    ...
    const mi = document.getElementById('menu-' + id), bn = document.getElementById('bn-' + id);
...
async function handleChatView() {
    ...
    const { data: msgs } = await supabase.from('mensagens').select('*').eq('vendor_uid', currentVendorUid).order('ts', { ascending: true });
    renderMessages(msgs || []);

    // Inscreve no Realtime para novas mensagens
    if (chatSub) supabase.removeChannel(chatSub);
    chatSub = supabase.channel(`chat_${currentVendorUid}`)
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'mensagens', filter: `vendor_uid=eq.${currentVendorUid}` }, payload => {
            const m = payload.new;
...
function createMessageHtml(m, isMe) {
    ...
    return `<div style="max-width:85%; padding:10px 14px; border-radius:16px; margin-bottom:10px; align-self:${isMe ? 'flex-end' : 'flex-start'}; background:${isMe ? 'var(--gold)' : 'var(--surface2)'}; color:${isMe ? 'var(--bg)' : 'var(--text)'}; font-size:0.9rem; box-shadow: 0 4px 10px rgba(0,0,0,0.1);">
                ${m.content || m.text}
...
async function sendChatMessage() {
    ...
    try {
        await supabase.from('mensagens').insert({
            vendor_uid: currentVendorUid, sender: currentVendorName, content: text, ts: new Date().toISOString()
        });
        input.value = '';
...
async function handleLogout() {
    try {
        if (currentVendorUid) await supabase.from('vendedores').update({ status: 'Offline' }).eq('uid', currentVendorUid);
...
async function handleAuth() {
    ...
    if (!rawUsername || !password) return showToast('Preencha os campos', 'error');

    showLoading(true);
    let email = rawUsername.includes('@') ? rawUsername : `${rawUsername.replace(/\D/g, '')}@unirotas.com`;
...
function startTracking() {
    ...
    watchId = navigator.geolocation.watchPosition(pos => {
        ...
        if (currentVendorUid) supabase.from('vendedores').update({ lat, lon, lastActive: new Date().toISOString() }).eq('uid', currentVendorUid);
        if (typeof recordGpsPoint === 'function') recordGpsPoint(lat, lon);
    }, e => showToast('Erro GPS', 'error'), { enableHighAccuracy: true });
}
...
// ── MEETING SCREENS (Integração com meeting-logic.js) ─────────────────────
...
async function loadMyMeetingHistory() {
    ...
    try {
        const { data: list, error } = await supabase.from('meeting_sessions')
            .select('*').eq('driver_id', currentVendorUid).order('date', { ascending: false }).limit(20);
        ...
        `).join('');
    } catch (e) { container.innerHTML = '<p style="color:#ff4757; text-align:center;">Erro ao carregar.</p>'; }
}
...
async function enterApp(uid) {
    ...
    try {
        const { data: profile } = await supabase.from('usuarios').select('*').eq('uid', uid).maybeSingle();
        if (profile) {
            currentVendorName = profile.name || 'Vendedor';
            window._currentUserId = uid; window._userProfile = profile;
            ...
            supabase.from('vendedores').upsert({ uid, status: 'Online', name: currentVendorName, lastActive: new Date().toISOString() });
        }
        initVendedorMap();
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    supabase.auth.onAuthStateChange((event, session) => {
        ...
        console.log("[UniRotas] Auth Event:", event, "AppActive:", isAppActive);
